...
public class CommentController {
    ...
    public ResponseEntity createComment(@PathVariable Long mateId,
                                        @Valid @RequestBody CommentRequest request) {
        log.info("메이트 모집글 댓글 등록");

        Member member = (Member) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        Long memberId = member.getMemberId();
        commentService.createComment(mateId, request, memberId);
    ...
    public ResponseEntity createReply(@PathVariable Long commentId,
        ...
        Member member = (Member) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        Long memberId = member.getMemberId();
        commentService.createReply(commentId, request, memberId);

        return new ResponseEntity<>("답글이 등록되었습니다.", HttpStatus.OK);
    }

    @GetMapping("/mates/{mateId}/comments")
    public ResponseEntity<CommentsResponse> findCommentsByMateId(@PathVariable Long mateId) {
        log.info("메이트 모집글의 댓글 목록 조회");

        CommentsResponse commentsResponse = commentService.findCommentsByMateId(mateId);
    ...
    public ResponseEntity removeComment(@PathVariable Long commentId) {
        ...
        Member member = (Member) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        Long memberId = member.getMemberId();
        commentService.removeByCommentIdAndMemberId(commentId, memberId);
